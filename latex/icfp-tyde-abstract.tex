...
% When we remove a binding that contains the only occurences of some other variable,
% we can run the optimization again to remove that binding as well.
% This is common among optimizations, which then can be iteratively applied until a fixpoint is reached.
...
A removed binding can contain the only occurences of some other variable.
This makes another binding dead, allowing further optimization when running the algorithm again.
...
in general it can be useful to simply iterate the optimization until a fixpoint is reached.
