...
\Todo{say something generally about open/closed terms etc.?}
...
the environment contains an element of the matching type,
so a $\AgdaDatatype{Ref}\ \sigma\ \Gamma$ always allows us to extract
a value of type $\sigma$ from $\AgdaDatatype{Env}\ \Gamma$:

\CodeLangRef

The type of expressions \AgdaDatatype{Expr} is indexed by both
its return type ($\sigma : \AgdaDatatype{U}$)
...
as opposed to working on contexts directly, e.g. $[\texttt{INT}] \subseteq [\texttt{INT}, \texttt{INT}]$.
...
Note that we can evaluate \AgdaDatatype{LiveExpr} directly, with the main difference
%WOUTER - scrap 'with' in the line above?
that in the \AgdaInductiveConstructor{Let}-case we match on $\Delta_2$ to distinguish whether the bound variable is live.
