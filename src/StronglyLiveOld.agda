...
--
-- Work in progress.
-- Outdated, incomplete and unused.
module StronglyLiveOld where
...
-- TODO: replace with sing?
all[_] : Ref τ Γ → SubCtx Γ
all[ Top ] = Keep ∅
all[ Pop i ] = Drop (all[ i ])

data LiveExpr (Γ : Ctx) : (Δ : SubCtx Γ) → (σ : U) → Set where
...
analyse : Expr Γ σ → Σ[ Δ ∈ SubCtx Γ ] LiveExpr Γ Δ σ
...
helper : (Δ₁ : SubCtx (σ ∷ Γ)) (Δ₂ : SubCtx Γ) → Expr ⌊ Keep {Γ} {σ} Δ₂ ⌋ τ → Expr (σ ∷ ⌊ pop Δ₁ ∪ Δ₂ ⌋) τ 
...
sdbe : Expr Γ σ → Σ[ Δ ∈ SubCtx Γ ] Expr ⌊ Δ ⌋ σ
...
liftRename : (Δ : SubCtx Γ) → Expr ⌊ Δ ⌋ σ → Expr Γ σ
...
lemma-lookup : (Δ₁ Δ₂ : SubCtx Γ) (i : Ref σ ⌊ Δ₁ ⌋) (env : Env ⌊ Δ₂ ⌋) → (subset : Δ₁ ⊆ Δ₂) →
...
prjEnv' : (Δ : SubCtx Γ) (env : Env Γ) → Env ⌊ Δ ⌋
...
analyse-preserves (App e₁ e₂) = {!!}
analyse-preserves (Lam e) = {!!}
